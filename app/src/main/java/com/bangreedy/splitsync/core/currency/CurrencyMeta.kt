package com.bangreedy.splitsync.core.currency

object CurrencyMeta {

    // Currency code → minor-unit digits. Default is 2 for unlisted currencies.
    private val minorUnits = mapOf(
        // 0-decimal currencies
        "BIF" to 0, "CLP" to 0, "DJF" to 0, "GNF" to 0, "ISK" to 0,
        "JPY" to 0, "KMF" to 0, "KRW" to 0, "KPW" to 0, "PYG" to 0,
        "RWF" to 0, "UGX" to 0, "VND" to 0, "VUV" to 0, "XAF" to 0,
        "XOF" to 0, "XPF" to 0,
    ...
    val supportedCurrencies: Map<String, String> = linkedMapOf(
        ...
        "ZWG" to "Zimbabwean ZiG",
        "ZWL" to "Zimbabwean Dollar",
    )

    fun getMinorUnits(currencyCode: String): Int {
        return minorUnits[currencyCode.uppercase()] ?: 2
    }

    /**
     * Convert a user-entered major amount to the app's centified storage format (×100).
     * This is currency-agnostic — we always store as ×100.
     */
    fun toMinor(majorAmount: Double): Long {
        return (majorAmount * 100).toLong()
    }

    /**
     * Convert the app's centified storage value back to a major amount.
     * This is currency-agnostic — stored values are always ×100.
     */
    fun toMajor(minorAmount: Long): Double {
        return minorAmount.toDouble() / 100.0
    }

    fun nameOf(code: String): String = supportedCurrencies[code.uppercase()] ?: code
}
